...
final class SdkGateTests: XCTestCase {
    ...
    func testInitializeGateExemptOnSimulator() async {
        await OneS1ghtSDK.reset()                      // 이전 테스트 세션 격리
        do {
            try await OneS1ghtSDK.initialize(sdkKey: "ock_gate_probe_invalid")
    ...
    func testAvailabilityIsHonestOnSimulator() {
        XCTAssertNotEqual(OneS1ghtSDK.positioningAvailability, .available,
                          "시뮬레이터에 UWB 가 있을 수 없음 — 조회가 거짓말 중")
        XCTAssertFalse(OneS1ghtSDK.isPositioningAvailable)
    }
}
