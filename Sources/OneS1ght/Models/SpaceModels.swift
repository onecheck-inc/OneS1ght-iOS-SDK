...
//  · 데이터 출처(콘솔/공간 서비스)는 SDK 내부 사정이라 타입 이름에 드러내지 않는다.
...
public struct Locator: Equatable {
    ...
    /// 서버가 이 로케이터를 **배치 완료로 보고 있는가**(공간 서비스 `cluster_status == "auto_done"`).
    ///
...
struct FloorState {
    ...
    let hasPlan: Bool
    /// 로케이터 **조회가 실패**했는가(통신·서버·404). 참이면 `locators` 는 비어 있지만
    /// 그것은 "층에 없다" 가 아니라 "못 받았다" 는 뜻이다 — 코드도 다르게 남긴다(E3006 vs E3002).
    /// ⚠️ 어느 쪽이든 **도면·존 표시는 막지 않는다.**
    let locatorsFetchFailed: Bool
}
