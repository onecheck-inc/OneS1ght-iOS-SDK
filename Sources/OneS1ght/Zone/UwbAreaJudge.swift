...
//  옛 존 엔진의 후신이다 — 판정 자체를 엔진이 하므로 좌표를 먹지 않는다.
//
//  종전(자체 존 엔진)과 같은 점:
...
//     옛 존 엔진은 그 값을 소비했지만 엔진은 자기 서버의 지오펜스로 판정한다. 콘솔에서 그 값을
//     바꿔도 판정이 변하지 않는다.
//   · 그래서 콘솔 존은 이제 **지도 표시·zone_id 매핑·서버 전송**을 위해서만 쓰인다.
//   · 엔진 영역 이름과 콘솔 존 이름이 어긋나면 이벤트가 버려진다 — 조용히 넘기지 않고
//     경고로 남긴다(엔진.areaUnmapped). 이름이 유일한 연결고리라 여기가 끊기면 시책이 안 돈다.
//
...
final class UwbAreaJudge {
    ...
    func apply(zones: [Zone]) {
        ...
        for z in zones {
            ...
            // (옛 존 엔진 과 같은 규칙 — 콘솔에서 같은 이름을 두 번 쓴 경우)
            var key = z.name
            var n = 2
    ...
    ///
    /// 옛 존 엔진 경로에서는 이 값들이 실제 판정에 들어갔다. 엔진 경로에서는 엔진이 자기 서버의
    ...
    ///    기본값 3.0 으로 채워져 내려와(SpaceServiceClient.RawZone), 설정한 것과 구분되지 않는다.
    ...
    private func warnIfJudgingParamsIgnored() {
        ...
        onLog?(.warn, SdkLocalized.format("uwb.paramsIgnored", zones.count))
    }
    ...
    /// - Parameters:
    ///   - inOut: 엔진이 주는 `"IN"` / `"OUT"` 문자열 (그 밖의 값은 무시)
    ///   - at: 발생 시각 (테스트용 시계 주입)
    func handleAreaEvent(inOut: String, areaName: String, at: Date = Date()) {
        guard let zone = nameToZone[areaName] else {
            ...
            if warnedNames.insert(areaName).inserted {
                onLog?(.warn, SdkLocalized.format("uwb.areaUnmapped", areaName, zones.count))
